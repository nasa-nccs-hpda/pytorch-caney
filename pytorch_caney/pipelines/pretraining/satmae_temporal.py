from pytorch_caney.data.datamodules.satmae_datamodule import SatMAEDataModule 

from pytorch_caney.training.mim_utils \
    import save_checkpoint

from pytorch_caney.models.vit_mae_temporal import MaskedAutoencoderViT

from pytorch_caney.training.mim_utils import get_grad_norm
from pytorch_caney.lr_scheduler import setup_scaled_lr
...
def execute_one_epoch(config,
    ...
    for idx, (img, timestamps, _) in enumerate(dataloader):
        ...
        with amp.autocast(enabled=config.ENABLE_AMP):
            loss, _, _ = model(img, timestamps, mask_ratio=config.mask_ratio)
        ...
        if config.TRAIN.ACCUMULATION_STEPS > 1:
            ...
            if (idx + 1) % config.TRAIN.ACCUMULATION_STEPS == 0:
                ...
                optimizer.zero_grad()
                scaler.update()
                lr_sched.adjust_learning_rate(optimizer, idx / num_steps + epoch)
...
def main(config):
    ...
    satmae_model = build_model(config, logger)

    model, model_wo_ddp = make_ddp(satmae_model)

    param_groups = optim_factory.add_weight_decay(model_wo_ddp, config.TRAIN.WEIGHT_DECAY)

    optimizer = torch.optim.AdamW(param_groups, lr=config.TRAIN.BASE_LR, betas=config.TRAIN.OPTIMIZER.BETAS)
...
def build_model(config, logger):
    ...
    model = MaskedAutoencoderViT(img_size=config.DATA.IMG_SIZE,
