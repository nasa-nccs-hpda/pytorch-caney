    ...
    if strategy == 'deepspeed':
        deepspeed_config = {
            ...
            "zero_optimization": {
                ...
                "contiguous_gradients": config.DEEPSPEED.CONTIGUOUS_GRADIENTS,
                "overlap_comm": config.DEEPSPEED.OVERLAP_COMM,
                "reduce_bucket_size": config.DEEPSPEED.REDUCE_BUCKET_SIZE,
                "allgather_bucket_size": config.DEEPSPEED.ALLGATHER_BUCKET_SIZE,
            },
...
def get_distributed_train_batches(config, trainer):
    if config.TRAIN.NUM_TRAIN_BATCHES:
        ...
        return config.DATA.LENGTH // (config.DATA.BATCH_SIZE * trainer.world_size) 
