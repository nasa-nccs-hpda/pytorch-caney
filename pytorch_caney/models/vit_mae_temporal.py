...
from pytorch_caney.network.pos_embed import (
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def random_masking(
        ...
        if self.same_mask:
            ...
            ids_shuffle_keep = [
                z[:, : int(L2 * (1 - mask_ratio))] for z in ids_shuffle
            ]
            ids_shuffle_disc = [
                z[:, int(L2 * (1 - mask_ratio)):] for z in ids_shuffle
            ]
    ...
    def forward_encoder(
        ...
        x = x + torch.cat(
            [
                self.pos_embed[:, 1:, :].repeat(ts_embed.shape[0], 3, 1),
                ts_embed,
            ],
            dim=-1,
        )
    ...
    def forward_loss(
        ...
        # viz code
        """
        m = torch.tensor([0.4182007312774658, 0.4214799106121063,
        0.3991275727748871]).reshape(1, 3, 1, 1)
        std = torch.tensor([0.28774282336235046, 0.27541765570640564,
        0.2764017581939697]).reshape(1, 3, 1, 1)
        ...
        previous_target = previous_target.reshape(bs, 3, -1,
        previous_target.shape[-1])[0]
...
if __name__ == "__main__":
    ...
    def mae_vit_base_patch16_dec512d8b(
        **kwargs,
    ):
    ...
    def mae_vit_large_patch16_dec512d8b(
        **kwargs,
    ):
        model = MaskedAutoencoderViT(
            patch_size=16,
    ...
    def mae_vit_large_patch16_dec512d8b_samemask(
        **kwargs,
    ):
        model = MaskedAutoencoderViT(
            patch_size=16,
    ...
    def mae_vit_huge_patch14_dec512d8b(
        **kwargs,
    ):
        model = MaskedAutoencoderViT(
            patch_size=14,
